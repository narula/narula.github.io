...
<b>Fees</b>. Decentralized blockchain networks charge transaction fees, which float according to market prices for blockchain space. The fee on Ethereum for a USDT ERC-20 transfer is currently $9.75 and for a USDC ERC-20 transfer is $10.25.[^8] Fees will rise if overall transaction volume on Ethereum grows--whether related to stablecoins or via other tokens or contracts--without a corresponding increase in transaction capacity.[^9] This might even unexpectedly render some stablecoins token accounts as unusable “dust”—-of a value where it is no longer economically viable to transfer them, given the fees on the blockchain to do so. Fees also can spike temporarily, which might make it hard for users to predict when they will be able to afford to transfer their tokens.[^10]
